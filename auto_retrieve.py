...
def query_graph(query_text, max_nodes=10):
    ...
    try:
        ...
        for node, score in matched_nodes:
            neighbors = []
            # Check both outgoing and incoming edges for DiGraph
            for neighbor in G.successors(node):
                edge_data = G.get_edge_data(node, neighbor, default={})
                edge_type = edge_data.get("relation", edge_data.get("type", "related"))
                neighbors.append(f"{neighbor} ({edge_type})")
            for neighbor in G.predecessors(node):
                edge_data = G.get_edge_data(neighbor, node, default={})
                edge_type = edge_data.get("relation", edge_data.get("type", "related"))
                neighbors.append(f"{neighbor} ({edge_type})")
